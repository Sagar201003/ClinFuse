...
class TemporalBuilder:
    def __init__(self, records: List[Dict[str, Any]]):
        self.timelines = self._build_patient_timelines(records)

    def _build_patient_timelines(self, records: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    def get_temporal_context(self, patient_id: str, current_study_id: str) -> List[Dict[str, Any]]:
        """
        Returns all records BEFORE the current study for a given patient (past history only).
        Assumes the timeline for the patient is already sorted chronologically.
        """
        if patient_id not in self.timelines:
            return []
            
        patient_records = self.timelines[patient_id]
        context_records = []
